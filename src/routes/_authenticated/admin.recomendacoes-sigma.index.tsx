      ...
      <PageHeader
        title={
          <span className="inline-flex items-center gap-2">
            <Star className="size-5 text-amber-400" /> Recomendações
          </span>
        }
      ...
      {q.isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {packs.map((p) => (
            <article
              key={p.id}
              className="group rounded-2xl border border-border/60 bg-card overflow-hidden hover:border-border transition"
            >
              <div className="aspect-[16/9] relative bg-gradient-to-br from-amber-500/20 via-rose-500/10 to-fuchsia-500/20">
                ...
                <div className="absolute top-2 right-2">
                  <span
                    className={`text-[10px] uppercase tracking-wider font-medium px-2 py-1 rounded-full backdrop-blur ${
                      p.is_published
        ...
        <div className="flex flex-col gap-3">
          {packs.map((p) => (
            <article
              key={p.id}
              className="group flex gap-4 rounded-2xl border border-border/60 bg-card p-3 hover:border-border transition"
            >
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-center gap-1.5">
                ...
                <div className="ds-meta">
                  {p.country ? `${p.country} · ` : ""}
                  {p.recs_count} pontos · {p.marketplace_count} marketplace · {p.faqs_count} FAQs ·{" "}
                  {p.adoption_count} guias usando
                </div>
              </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação remove a cidade e todos os pontos, FAQs e links de marketplace dela. Guias
              que estavam usando esta recomendação serão desvinculados (o conteúdo original deles é
              preservado).
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function Metric({
  ...
  return (
    <div
      className={`rounded-lg py-1.5 ${highlight ? "bg-amber-500/10 ring-1 ring-amber-400/20" : "bg-muted/30"}`}
    >
    ...
    >
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Cada cidade tem apenas uma recomendação. Comece em rascunho e publique quando estiver
            pronta.
          </DialogDescription>
        </DialogHeader>
          ...
          <div>
            ...
            {duplicate && (
              <p className="ds-meta text-rose-400 mt-1.5">
                Já existe uma recomendação para <strong>{duplicate.city_label}</strong>. Edite a
                cidade existente.
              </p>
            )}
          </div>
