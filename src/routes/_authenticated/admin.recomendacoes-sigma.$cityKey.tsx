...
import {
  ...
  PlaceAutocomplete,
  type RecItem,
} from "@/routes/_authenticated/admin.properties.$id";
...
import {
  ...
  deleteSigmaRecs,
  addSigmaMarketplace,
  updateSigmaMarketplace,
  deleteSigmaMarketplace,
  addSigmaFaq,
  updateSigmaFaq,
  deleteSigmaFaq,
  adminListPublishedGuidesForSigma,
  adminApplySigmaPackToProperty,
...
function SigmaPackEditor() {
  ...
  if (!q.data)
    return <div className="p-8 text-muted-foreground text-sm">Cidade não encontrada.</div>;
  ...
  async function togglePublish() {
    try {
      await updatePackFn({
        data: { city_key: cityKey, patch: { is_published: !pack.is_published } },
      });
    ...
    <div className="max-w-[1440px] mx-auto w-full px-6 lg:px-10 py-8 lg:py-10 space-y-8">
      ...
      <Tabs defaultValue="recs">
        <TabsList className="ds-segmented h-auto rounded-full">
          <TabsTrigger value="recs" className="rounded-full">
            <MapPin className="size-3.5" /> Pontos ({recs.length})
          </TabsTrigger>
          <TabsTrigger value="mkt" className="rounded-full">
            <Link2 className="size-3.5" /> Marketplace ({marketplace.length})
          </TabsTrigger>
          <TabsTrigger value="faqs" className="rounded-full">
            <HelpCircle className="size-3.5" /> FAQs ({faqs.length})
          </TabsTrigger>
        </TabsList>
  ...
  return (
    <>
      <Button
        ...
        title={disabled ? "Publique antes de aplicar" : "Aplicar em um Guia"}
      >
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Escolha um guia publicado/ativo desta cidade. O conteúdo SigmaConcierge substituirá
              pontos da cidade e marketplace, mantendo FAQs manuais editáveis.
            </DialogDescription>
          </DialogHeader>
          <div className="max-h-[55vh] overflow-y-auto space-y-2 pr-1">
            {q.isLoading && (
              <div className="text-sm text-muted-foreground py-6">Carregando guias…</div>
            )}
            {!q.isLoading && (q.data ?? []).length === 0 && (
              <div className="text-sm text-muted-foreground py-6">
                Nenhum guia publicado/ativo encontrado para esta cidade.
              </div>
            )}
            {(q.data ?? []).map((g) => {
              ...
              return (
                <div
                  key={g.id}
                  className="rounded-xl border border-border/60 bg-card/60 p-3 flex items-center gap-3"
                >
                  {g.hero_image_url ? (
                    <img
                    ...
                    <div className="size-11 rounded-lg bg-muted grid place-items-center">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="ds-meta truncate">
                      {g.city}
                      {g.state ? `/${g.state}` : ""} · {g.owner_email ?? "sem e-mail"}
                    </p>
                  </div>
                  <Button
                    size="sm"
                    className="rounded-full"
                    onClick={() => apply(g.id)}
                    disabled={!sameCity || applyingId === g.id}
                  >
...
function RecsTab({
  ...
  cityKey: string;
  items: Awaited<ReturnType<typeof adminGetSigmaPack>>["recs"];
  refresh: () => void;
}) {
  ...
  function scheduleUpdate(id: string, patch: Record<string, unknown>) {
    ...
    const t = setTimeout(() => {
      ...
      updFn({ data: { id, patch } })
        .then(refresh)
        .catch((e) => toast.error(friendlyErrorMessage(e, "Não foi possível salvar agora.")));
    }, 700);
    map.set(id, t);
  }
  ...
  function handleChange(next: RecItem[]) {
    ...
    if (deletedIds.length) {
      delFn({ data: { ids: deletedIds } })
        .then(refresh)
        .catch((e) => toast.error(friendlyErrorMessage(e, "Não foi possível excluir agora.")));
    }
  ...
  const existingPlaceIds = new Set(
    localItems.map((i) => i.place_id).filter((x): x is string => !!x),
  );
...
function MarketplaceTab({
  ...
  cityKey: string;
  items: Awaited<ReturnType<typeof adminGetSigmaPack>>["marketplace"];
  refresh: () => void;
}) {
      ...
      <div className="rounded-2xl border border-border/60 bg-card p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <Input
            ...
            placeholder="Nome (ex: Airbnb Experiences)"
          />
        ...
        <Textarea
          ...
          placeholder="Descrição curta (100–200 caracteres)"
        />
          ...
          items.map((m) => (
            <div key={m.id} className="rounded-xl border border-border/60 bg-card p-3 space-y-2">
                ...
                <input
                  ...
                  onBlur={(e) => {
                    if (e.target.value !== m.label)
                      updFn({ data: { id: m.id, patch: { label: e.target.value } } })
                        .then(refresh)
                        .catch(() => {});
                  }}
              ...
              <input
                defaultValue={m.url}
                onBlur={(e) => {
                  if (e.target.value !== m.url)
                    updFn({ data: { id: m.id, patch: { url: e.target.value } } })
                      .then(refresh)
                      .catch(() => {});
                }}
              ...
              <textarea
                ...
                onBlur={(e) => {
                  if (e.target.value !== (m.description ?? ""))
                    updFn({ data: { id: m.id, patch: { description: e.target.value || null } } })
                      .then(refresh)
                      .catch(() => {});
                }}
...
function FaqsTab({
  ...
  cityKey: string;
  items: Awaited<ReturnType<typeof adminGetSigmaPack>>["faqs"];
  refresh: () => void;
}) {
  ...
  async function add() {
    ...
    try {
      await addFn({
        data: { city_key: cityKey, question: question.trim(), answer: answer.trim(), tags: [] },
      });
      setQuestion("");
      setAnswer("");
                ...
                <input
                  ...
                  onBlur={(e) => {
                    if (e.target.value !== f.question)
                      updFn({ data: { id: f.id, patch: { question: e.target.value } } })
                        .then(refresh)
                        .catch(() => {});
                  }}
                  className="flex-1 bg-transparent text-sm font-medium outline-none"
              ...
              <textarea
                ...
                onBlur={(e) => {
                  if (e.target.value !== f.answer)
                    updFn({ data: { id: f.id, patch: { answer: e.target.value } } })
                      .then(refresh)
                      .catch(() => {});
                }}
                className="w-full bg-transparent text-xs text-muted-foreground outline-none resize-none"
