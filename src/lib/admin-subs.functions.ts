...
export const adminListAuditLogs = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ logs: AuditLogRow[] }> => {
    ...
    const rowList = (rows ?? []) as Array<{ id: string; user_id: string | null; user_email: string | null; action: string; entity_type: string | null; entity_id: string | null; metadata: Record<string, unknown>; created_at: string }>;
