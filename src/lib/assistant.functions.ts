...
function instructions(params: { knowledge: string; currentPath: string | null; today: string }): string {
  return [
    ...
    "· Você não grava: as ferramentas `preparar_*` montam a ação e a pessoa confirma no cartão. Depois de preparar, diga o que vai acontecer e peça a confirmação — nunca diga que já foi feito.",
    "· Um pedido, um cartão: prepare UMA ação por resposta. Se pediram várias, faça a primeira e ofereça a próxima.",
