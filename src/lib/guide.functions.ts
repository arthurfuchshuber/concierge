...
export const getPublicGuide = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: prop, error } = await supabaseAdmin
      .from("properties")
      .select("id,slug,name,tagline,hero_image_url,gallery_images,theme_images,address,maps_url,lat,lng,city,country,checkin_time,checkin_time_max,checkout_time,checkout_time_min,address_note,checkin_instructions,checkout_instructions,checkin_media,access_instructions,access_media,access_video_url,host_name,brand_name,brand_logo_url,access_mode,pin_expires_at,default_language,guide_theme,published,created_at,updated_at")
      .eq("slug", data.slug)
      .eq("published", true)
      .maybeSingle();
