  ...
  async function send(overrideText?: string) {
    ...
    try {
      ...
      if (errorMsg) {
        ...
        /* SEM AVISO DE TRANSFERÊNCIA (11/09/2026).
         *
         * Aqui era injetado um balão de sistema: "Um atendente humano vai
         * responder por aqui em instantes." Era o ÚNICO lugar em que o hóspede
         * lia sobre transferência — e contradizia a regra do produto e o
         * próprio prompt da IA, que proíbe anunciar equipe ou repasse.
         *
         * O balão existia para tapar um buraco real: nesse caminho o servidor
         * não devolvia texto nenhum. O buraco foi fechado no orquestrador, que
         * agora sempre responde alguma coisa na voz da IA. Quando ainda assim
         * vier vazio (erro de rede no meio do turno), o certo é não inventar
         * um aviso: o polling traz a próxima mensagem e a conversa segue. */
        setHumanMode(true);
        finishWith(
          acc.trim() ? [...next, { role: "assistant" as const, content: acc, quickReplies }] : next,
          convId,
        );
      } else {
              ...
              {m.role === "user" ? (
                ...
                <div className="text-[13.5px] leading-relaxed text-zinc-800 prose prose-sm max-w-none [&_p]:my-1 [&_p]:leading-relaxed [&_strong]:font-semibold [&_strong]:text-zinc-900 [&_a]:text-emerald-700 [&_a]:underline [&_a]:underline-offset-2 [&_ul]:my-1.5 [&_ul]:pl-4 [&_ol]:my-1.5 [&_ol]:pl-4 [&_li]:my-1 [&_li]:pl-0.5 [&_li>p]:my-0.5 [&_h1]:text-[14.5px] [&_h2]:text-[14.5px] [&_h3]:text-[14.5px] [&_h4]:text-[14px] [&_h1]:font-semibold [&_h2]:font-semibold [&_h3]:font-semibold [&_h4]:font-semibold [&_h1]:text-zinc-900 [&_h2]:text-zinc-900 [&_h3]:text-zinc-900 [&_h4]:text-zinc-900 [&_h1]:mt-3 [&_h2]:mt-3 [&_h3]:mt-3 [&_h4]:mt-3 [&_h1]:mb-1 [&_h2]:mb-1 [&_h3]:mb-1 [&_h4]:mb-1 [&_h1:first-child]:mt-0 [&_h2:first-child]:mt-0 [&_h3:first-child]:mt-0">
                  {/* O selo "Atendente" foi removido em 11/09/2026.
                      ...
                      em cada balão. A origem continua registrada em
