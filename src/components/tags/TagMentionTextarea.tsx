...
import { GUIDE_TAGS, GUIDE_INFOS, type GuideTagKey, type GuideInfoKey } from "@/lib/guide-tags";
...
export const TagMentionTextarea = forwardRef<TagMentionTextareaHandle, Props>(
  function TagMentionTextarea(
    {
      value,
      onChange,
      items = [],
    ...
    const options = useMemo<Option[]>(() => {
      const sections: Option[] = GUIDE_TAGS.filter(
        (t) => !t.parameterized || t.key === "faq" || t.key === "explorar",
      ).map((t) => ({
        ...
        {open && filtered.length > 0 && (
          ...
          >
            <div className="px-3 py-1.5 border-b flex items-center gap-2 text-[11px] text-muted-foreground">
              <TagIcon className="size-3.5" />@ para linkar seção, inserir valor (Wi-Fi, horários…)
              ou item · ↑↓ · Enter
            </div>
            {filtered.map((o, i) => (
              <button
