...
export function OperationWorkspace({ view }: { view: OperationView }) {
  ...
  const skipTaskOccurrenceMutation = useMutation({
    ...
    onSuccess: (res) => {
      invalidateTasks();
      notifyTaskUndo(`Ocorrência pulada. Próximo prazo: ${fmtDateBR(res.dueDate)}.`, restoreTaskUndo(res.before));
    },
...
=======
    <div className="mb-3 flex items-center gap-2.5">
      <span
        ...
        className="size-1.5 shrink-0 rounded-full bg-gradient-to-br from-[#7C1AD8] to-[#E82DAE]"
      />
      <span className="ds-eyebrow shrink-0 text-[10px] tracking-[0.2em] text-muted-foreground">Guia do hóspede</span>
      <span
        aria-hidden
        className="h-px flex-1 bg-gradient-to-r from-[color-mix(in_oklab,var(--foreground)_9%,transparent)] to-transparent"
      />
      <span className="shrink-0">
>>>>>>> c8b07cb893245d2181489d3ac4f02fbcba3be9f4
        <InfoHint title="Guia do hóspede">
          Conta os hóspedes com check-in no período. “Viram instruções de check-in” são os que já abriram as Instruções
          da sessão “Chegada” pelo menos uma vez; “Viram senha de acesso” são os que já visualizaram as senhas no guia
          pelo menos uma vez.
        </InfoHint>
      }
    />
...
=======
                  className={`ds-3d ds-3d-hover flex h-11 flex-1 items-center justify-center gap-2 rounded-[13px] bg-card text-[12.5px] font-bold transition-colors lg:size-11 lg:flex-none lg:gap-0 ${
                    cleaningWindow === "next" ? "ds-atencao" : "text-muted-foreground hover:text-foreground"
                  }`}
                >
    ...
    <div className="space-y-3">
      <div className="lg:flex lg:items-end lg:justify-between lg:gap-8">
        ...
        <div className="mt-5 flex flex-col gap-2 lg:mt-0 lg:flex-row lg:items-center lg:gap-2">
          ...
          {actions && (
            <div className="order-2 flex w-full items-center gap-2 lg:order-1 lg:w-auto lg:shrink-0">{actions}</div>
          )}
        </div>
      </div>
    </div>
...
=======
      ...
      <div className={`flex items-end justify-between gap-2 ${note && !loading ? "mt-[22px] sm:mt-5" : "mt-1.5"}`}>
