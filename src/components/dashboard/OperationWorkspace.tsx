...
export function OperationWorkspace({ view }: { view: OperationView }) {
  ...
  const occupancyProperties: Array<{
    id: string;
    name: string;
    city: string | null;
      ...
      onRevert:
        colMode === "checkin"
          ...
          : (row: ArrivalRow) => {
              ...
              revert.mutate({
                ...target,
                from: colMode as "checkout" | "stay" | "cleaning" | "done" | "no_show",
              });
            },
...
export function OperationShell({
  ...
  return (
    /* NO COMPUTADOR, as abas ficam na MESMA LINHA do título, à direita — é o
       ...
       `ds-page-heading` aqui e de novo na coluna do título: é a REGRA do
       respiro do cabeçalho (ver `styles.css`). Os dois únicos vãos deste
       bloco — data→título e título→subtítulo — saem da mesma variável, então
       não há como um mudar sem o outro. */
    <div className="ds-page-heading">
      {/* A DATA FICA FORA DA COLUNA DO TÍTULO (pedido explícito, 18/09/2026,
          com print: "essa data em uma subpágina está em linha inteira, e em
          outra não").

          Ela morava dentro do bloco `lg:flex-1` do título. Esse bloco divide a
          linha com as abas E com os botões de ação — e cada aba tem um número
          diferente de botões (o Kanban tem dois, o Operacional nenhum). Com
          dois botões a mais, a coluna encolhia ~100px e a data quebrava em
          duas linhas só ali, deslocando o título para baixo em relação às
          outras abas.

          Acima da linha inteira, ela tem SEMPRE a largura da página, não
          importa quantos botões a aba tenha. A posição na tela é a mesma de
          antes (canto superior esquerdo, acima do título); o que muda é que
          ela deixou de depender de quem divide a linha de baixo.

          A data em amarelo fraco (pedido explícito, 18/09/2026) — era o rosa
          da marca, que no topo da página disputava com a aba ativa. */}
      <p className="ds-eyebrow ds-data text-[10.5px] tracking-[0.2em]">{hoje}</p>
      <div className="lg:flex lg:items-end lg:justify-between lg:gap-8">
        {/* A MESMA `ds-page-heading` do bloco de fora: é isto que faz o vão
            título→subtítulo ser, por construção, o mesmo vão data→título. */}
...
export function TasksDialog({
  ...
  /**
   * Padrão = IMÓVEL, que é o que os dois mockups aprovados mostram selecionado.
   ...
   */
  const [groupBy, setGroupBy] = useState<TaskGroupBy>("property");
              ...
              <div ref={outerRef} className="w-full">
                ...
                >
                  <table
                    ...
                    style={{
                      width: nameColW + dayList.length * dayW,
                      minWidth: nameColW + dayList.length * dayW,
                    }}
                  >
                    <thead>
                      <tr>
...
function EngagementBars({
  ...
  return (
    ...
    <div className="flex items-start gap-3.5 lg:gap-7">
      {temCheckin && (
        <RingCell
          label="Viram instruções Check-In"
          value={checkinViewed}
          total={checkins}
...
function ArrivalCard({
  ...
  const cleaningChecklist = useMemo(() => {
    ...
    return cleaningTasks.tasks
      ...
      .map((t) => ({
        task: t,
        done: t.logId || t.reservationId ? t.status === "done" : completedHere.has(t.id),
      }))
